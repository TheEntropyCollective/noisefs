...
import (
	...
	"github.com/TheEntropyCollective/noisefs/pkg/testing"
)
...
func main() {
	...
	harness := testing.NewRealIPFSTestHarness(config)
...
func runSingleNodeBenchmark(harness *testing.RealIPFSTestHarness, fileSize, numFiles int, verbose bool, results *BenchmarkResults) error {
...
func runCrossNodeBenchmark(harness *testing.RealIPFSTestHarness, fileSize, numTests int, verbose bool, results *BenchmarkResults) error {
...
func runCacheEfficiencyBenchmark(harness *testing.RealIPFSTestHarness, fileSize int, verbose bool, results *BenchmarkResults) error {
	_, err := harness.GetNode(0)
	if err != nil {
		return fmt.Errorf("failed to get node: %w", err)
	}
	...
	fmt.Println("  Simulating cache efficiency testing...")
...
func runConcurrentBenchmark(harness *testing.RealIPFSTestHarness, fileSize, numFiles int, verbose bool, results *BenchmarkResults) error {
	nodes := harness.GetAllNodes()
